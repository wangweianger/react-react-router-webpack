...
class common{
	...
	*/
	ajax(json){
		...
		$.ajax({
			...
			data: json.data||"",
			datatype:"json",
			async: typeof(json.async)=='boolean'?json.async:true,
	...
	*/
	...
	data 附加的data值
	*/
	updateFileFormData(json){
		var This=this;
		This.showLoading();
	    var formData = new FormData(json.formname[0]); 
	    if(json.data){
		    for(var i in json.data){
		    	formData.append(i, json.data[i]);
		    };
	...
	*/
	...
	callback
	*/
	gitPreSityInfor(json){
		this.ajax({
			url:config.baseApi+'mainOrder/queryArea',
			data:{
