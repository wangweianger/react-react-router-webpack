	...
	this.parent.find('*').each(function(){
		...
		if(type){
			...
			$(this).bind(This.settings.event,function(){	 
				...
				var reg=common.regCombination(zimu,numArr);         //生成正则表达式
	...
	if(obj.attr("ignore")&&value || !obj.attr("ignore")){   //当内容为空 或者没有ignore 属性的时候执行
		...
		if(obj.attr("recheck")){
			if(value !=($("[name="+obj.attr("recheck")+"]").val())){
				if(nodeName=="input" || nodeName=="select"){
					...
					obj.parent().append("<span class='red tiniestsize glyphicon glyphicon-remove'>"+obj.attr("errormsg")+"</span>");
				}
		...
		if(reg.test(value)){
			if(nodeName=="input" || nodeName=="select"){
				obj.parent().find("span").remove();
				if(obj.attr("sucmsg")){
					obj.parent().append("<span class='green tiniestsize glyphicon glyphicon-ok'>"+obj.attr("sucmsg")+"</span>");
				}
			}
